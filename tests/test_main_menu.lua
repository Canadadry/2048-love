...
test("down key moves cursor to 1 (Quit)", function()
    local s = gamestate.new()
    s:keypressed("down")
    eq(s:menu_cursor(), 1, "cursor at 1 after down")
end)

test("down key clamps cursor at 1", function()
    local s = gamestate.new()
    s:keypressed("down")
    s:keypressed("down")
    eq(s:menu_cursor(), 1, "cursor clamped at 1")
end)
...
test("enter with cursor=1 (Quit) sets quit_requested", function()
    local s = gamestate.new()
    s:keypressed("down")
    s:keypressed("return")
