...
When you create a 2048 tile a **You Win** screen slides in with three options — **Continue** (keep playing), **Restart**, and **Main Menu** — navigated with Up/Down and confirmed with Enter, or tapped directly. A burst of 50-100 colored squares explodes across the screen and falls under gravity (suppressed when the **Effects** option is off). A **Game Over** screen appears when no moves remain with two options — **New Game** and **Main Menu** — activated by pressing Enter, any arrow key, or tapping.

Press `Escape` (or tap the **⏸** button above the board's top-right corner) to open the **Pause** screen. Up/Down to navigate, Enter to confirm. Options: **Resume**, **New Game**, **Main Menu**, **Quit**. Pressing `Escape` again or selecting Resume returns to the game.

All screen changes animate with a push transition — the new screen slides in as the old one slides out, mirroring the tile-push mechanic of the board.

Select **Options** from the main menu (between New Game and Quit) to open the **Options** screen — rows: **Win Tile**, **Theme**, **Animations**, **Effects**, **Sound**, **Language**, **Back**. Up/Down moves focus between the rows (wrapping at both ends); the focused row is highlighted. Left/Right cycles the focused row's value immediately — no confirm step (a no-op on Back, which has no value). Tapping an unfocused row focuses it; tapping the already-focused row cycles it forward (wrapping), same as Right — for Back, that second tap activates it. **Win Tile** toggles between `32` (dev) and `2048` (prod) — the same value `--win-tile` sets at launch, but changeable mid-session. **Theme** cycles through the built tilesheets in `game/assets/` (plus a "None (classic)" entry). **Animations** toggles the slide animation on/off — when off, moves snap to their destination instantly. **Effects** toggles the merge pop/scale effect on/off, independent of slide animation. **Language** cycles through available languages; names appear in their native form (e.g. "Français"). Enter has no effect except on the Back row, where it returns to the main menu. Press `Escape` from any row, or focus and confirm **Back** (Enter or a second tap), to return to the main menu.

All six settings persist across launches — they're saved to disk as soon as you change them, and restored on the next launch. `--win-tile` at launch still overrides a saved Win Tile value.
